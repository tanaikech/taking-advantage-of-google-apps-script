...
- [サンプルスクリプト](#samplescripts)
  ...
  - [A2UI: エージェントとユーザー間のインターフェース](#a2ui)
...
### [次世代Google Workspace自動化](https://gist.github.com/tanaikech/22963f5878058d981e9584039a30d91a)

> 受動的なチャットボットから自律的な実行環境への移行は、Google Cloud Next '26でGemini Enterprise Agent Platformが発表されたことで確固たるものとなりました。本稿では、Google Workspace自動化のための最先端のAIエージェント手法4つを評価します。これらの手法は、著名な開発者であるMartin Hawksey氏、Bruce Mcpherson氏、Kanshi Tanaike氏によって開発されました。CLIスキルチェーン、高度なエミュレーションサンドボックス、動的コード生成、A2Aリモート委任といった構造的なアプローチを分析し、これらのコミュニティ主導のイノベーションが、公式のエージェントスキルリポジトリやモデルコンテキストプロトコル（MCP）サポートといったNext '26のネイティブ機能をどのように先取りしていたかを示します。これらの基盤の上に、ゼロトラストのマルチクラウドタスクルーティングを実現するフェデレーテッドコンテキスト認識ルーティングアーキテクチャ（Federated CARA）と、動的なセマンティックキャッシングを用いてツール空間の干渉を排除する自己最適化ツールキャッシングネットワーク（SOTCN）という、2つの新しいフレームワークを提案します。この比較分析では、既存モデルと提案モデルをGoogleの新しいエンタープライズ標準に照らし合わせ、安全で高度に動的なマルチエージェントオーケストレーションのための拡張可能なロードマップを提供します。

### [エージェント・アズ・ア・ツール：AIオーケストレーションの新時代](https://gist.github.com/tanaikech/8851fd6580f7c73254aea38620db8b33)

> 大規模言語モデル（LLM）エージェントが多数の外部システムを統合するにつれて、ツール空間干渉（TSI）という問題が生じます。これは、コンテキストの肥大化、アテンションの希釈、推論精度の低下を引き起こす現象です。本稿では、最近提案された自己最適化ツールキャッシングネットワーク（SOTCN）とフェデレーテッドコンテキスト認識ルーティングアーキテクチャ（Federated CARA）の進化的な実用的実装である、エージェント・アズ・ア・ツールパラダイムを紹介します。このアーキテクチャは、検索拡張生成（RAG）を活用して、ステートフルで自律的なサブエージェントを動的に発見し、オンザフライで組み立てることで、TSIを完全に排除し、ゼロトラストの実行境界を強制し、無限に拡張可能なAIオーケストレーションを実現します。

### [開発者向けエージェントスキル理解ガイド：Google Apps Scriptにおけるプログレッシブ・ディスクロージャーの実装](https://gist.github.com/tanaikech/584931d08082be798cd5f3b893c0c44c)

> 大規模言語モデルが自律型エージェントへと進化するにつれ、開発者はツール空間干渉（TSI）に直面します。これは、過剰なツールによってコンテキストが肥大化し、推論能力が低下する現象です。本稿では、手続き型知識をカプセル化するための決定的なソリューションとして、エージェントスキルについて考察します。スキルはモデルコンテキストプロトコル（MCP）と競合するのではなく、エージェントのオンボーディングガイドとして機能することでMCPを補完します。3段階のプログレッシブ・ディスクロージャー・アーキテクチャにより、複雑な命令を必要に応じてのみ公開することで、トークンの使用を最適化します。Google Apps Scriptを用いた、スケーラブルなGemini CLIにインスパイアされたワークフローを実演し、動的なコード実行と、信頼性の高いエンタープライズグレードのAIエコシステムを構築するための重要なセキュリティ上の考慮事項を解説します。

### [Google Apps Script 用エージェント開発キット](https://gist.github.com/tanaikech/653dfe16b470f962131606191c289de4)
...
### [Vibeコーディングを使用した複雑なスケジュールでのGoogle Apps Scriptの実行](https://gist.github.com/tanaikech/2e40783d6584e65482667a22718f959b)

> Google Apps Scriptで、平日の午前中のみタスクを実行するなど、複雑な時間駆動型トリガーを設定することは、プログラムによる実装が非常に困難であり、標準UIでは事実上不可能です。TriggerAppは、宣言型JSONエンジンによってこのアーキテクチャ上の課題を解消し、開発者が細かな日付計算ロジックを完全に回避できるようにします。そして今回、ネイティブのModel Context Protocol（MCP）サーバーを組み込むことで、決定的なパラダイムシフトを実現しました。開発者は、生成AI（Vibe Coding）を介して自然言語を使用して複雑で継続的にループするGASスケジュールを調整し、洗練された再帰的なデイジーチェーンアーキテクチャによって20トリガーのクォータ制限を維持し、将来の実行バッチを動的にキューイングすることによって厳格な6分間の実行タイムアウトを回避できます。

### [1秒タイムアウトハック：Google Apps Scriptでネイティブに無限並列ワークロードを実行する](https://gist.github.com/tanaikech/343a86fbe631fdc3eda0987d5bb1f44a)

> 本稿では、Google Apps Script（GAS）のステートレス性と6分間の実行制限を克服するサーバーレスアーキテクチャを提案します。UrlFetchAppループバック呼び出しに1秒の即時タイムアウトを設定することで、オーケストレーターはバックグラウンドタスクをディスパッチし、即座に終了します。この設計により、ターゲットWebアプリケーションが隔離されたコンテナ内で完了するまで、呼び出し元の実行クォータが解放されます。トランザクション対応のGoogle Sheetsステートマシンと組み合わせることで、この設計は、外部コンピューティングインフラストラクチャなしで、自己永続的な並列MapReduce実行と、マルチターンで状態をハイドレートする生成型AIエージェントネットワークをサポートします。
...
### [LLMの「忘却」を防ぐ（パート2）：AIの「関係性の盲点」を解消するGraph-Anchor Pyramid（GAP）](https://gist.github.com/tanaikech/7be4176044daae6bae18b591007ab290)

> 本記事では、長大なコンテキストにおけるAIの「関係性の盲点」を解消する高度なRAGフレームワーク「GAP（Graph-Anchor Pyramid）」を紹介します。GAPは、Graph-RAGによる検索、トポロジカルなリーフ（末端ノード）のグルーピング、そしてプロンプトレベルのセマンティック・アンカーを組み合わせることで、階層的要約において因果関係の連鎖を100%の精度で再現（リコール）します。これにより、重要な事実を損なうことなく、膨大なデータを並列かつ効率的に処理することが可能になります。

### [Robust-GAP: 階層型RAGにおけるハルシネーション・ゼロの因果関係要約の実現](https://gist.github.com/tanaikech/758232a5d5399582eaa8f3d67b4e394e)

> 本記事では、複数の文書からなるログの要約処理において、意味的なハルシネーション（事実と異なる生成）や知識の乖離（ドリフト）を排除するために設計された、階層型RAG（検索拡張生成）フレームワーク「Robust-GAP」を紹介します。このフレームワークは、動的因果グラフ抽出（DLCE）、能動的トポロジー検証（SGAV）、およびメタデータ・プロベナンス伝播（PAPP）を組み合わせることで、厳密な引用の追跡可能性を確保し、LLMによるハルシネーションの発生を防止します。

### [バーチャルR&Dラボの内部：人間の想像力とAIマルチエージェントが形作る科学の未来](https://gist.github.com/tanaikech/060a86fde7577639a098655a8b40bf1d)

> 本事例では、Antigravity CLI上でGeminiと`tanaike-lab`を活用し、都市部の豪雨に関する流体解析の研究プロジェクトを「ゼロから1」へと立ち上げたプロセスを紹介します。システムの秩序、AIによる論理の加速、そして人間の明確な想像力を融合させることで、暗闇の深淵から新たな科学的現実を導き出す、次世代のR&Dパラダイムを実証します。

### [地震発生前：Antigravity CLIのAIエージェントとIoTデータによる地震予測](https://gist.github.com/tanaikech/b0c336e5fa73d7edf85e6f884d730536)
...
### [生成AI時代におけるGoogle Apps Scriptの役割の再定義](https://gist.github.com/tanaikech/acc33326c86dce03bc2328ccb12f6b04)

> 生成AIや自律型エージェントの登場によってGoogle Apps Script (GAS) が不要になるわけではありません。むしろ、GASは「確定的（デターミニスティック）な実行基盤」として、より不可欠な存在へと進化します。本記事では、AIによる確率的な推論（「脳」の役割）と、GASによる安全かつ低コストなイベント駆動型実行（「神経系」の役割）の間で役割を分担する、エンタープライズ向けのハイブリッド・アーキテクチャを提示します。MCPサーバー、確定的なガードレール、ハイブリッド・バッチ処理など、12の実運用ユースケースを通じて、堅牢かつスケーラブルなGoogle Workspace自動化を構築するための4つの基本原則を体系化します。

### [Google Apps ScriptでGemini Managed Agentsを活用する](https://gist.github.com/tanaikech/0f71ec60332df260a04a60f425ef4829)

> Google Apps Script (GAS) はGoogle Workspaceの自動化に強力なツールですが、プラットフォームや計算リソース上の制約により、高度な処理を行うのが難しい場合があります。一方、Gemini Managed Agentsは、bashの実行環境を備えたリモートLinuxサンドボックスを提供します。本記事では、GASとLinuxサンドボックスを連携させ、Apps Script単体では対応困難なタスクを実行するためのアーキテクチャを紹介します。Linuxサンドボックス内で生成された成果物をGoogle Driveへ直接ストリーミングすることで、APIのペイロード制限やトークン処理のオーバーヘッドを回避し、高スループットなクラウド自動化を実現します。

### [山の咆哮を聴く：Antigravity CLIのAIエージェントとIoTデータが捉えた火山衝撃波](https://gist.github.com/tanaikech/80710d971718f884d327e33e765436de)

> 爆発的な火山噴火は、地球の最下層大気（境界層）に閉じ込められ、音速で地球を水平伝播する強力な大気音響波「大気ラム波（Lamb wave）」を励起します。Google Apps Script（GAS）が20分毎に気象データをGoogle Driveへ自動記録するバックグラウンド収集体制のもと、私たちは2つのベンチマーク噴火イベント（2018年の草津白根山水蒸気爆発および新燃岳マグマ噴火）を照合・適応させました。そして、**Gemini** と自律型仮想研究室フレームワーク **`tanaike-lab`** を搭載した **Antigravity CLI** 環境内で、これらの現象を第一原理連続体力学に結合しました。日本全国の29,334件の真正なNetatmo観測ビッグデータを通じて、本フレームワークは氷点下の山岳から春の平地までの気温に依存する衝撃波見かけ伝播速度（秒速304.38メートルおよび313.27メートル、理論音速一致度98.5%以上）を再構成し、未監視の火口方位を誤差1.78度以内で特定し、爆発力学エネルギー（TNT火薬換算178.8〜1,041.1トン相当）を定量化し、猛烈な暴風雨下でも**誤報ゼロ（偽陽性率0.0%）**を保ちながら**2.5〜15分前の事前早期警報リードタイム**を実現しました。本成果は、生成AIを第一原理物理学に接地させることが、いかに堅牢な惑星センシングシステムを生み出すかを実証するものです。
...
- [A2UI、Gemini、Google Apps Script を使用した適応型学習エージェントの構築](https://gist.github.com/tanaikech/0f26de9e861353ef95caa8ec50411087)
